...
func _ready() -> void:
	...
	# Hide joystick if needed based on visibility_mode.
...
func _input(event: InputEvent) -> void:
	# Process screen touch events.
	if event is InputEventScreenTouch:
		if event.pressed:
			# Only start if no other touch is already active.
			if _touch_index == -1 and _is_point_inside_joystick_area(event.position):
				# For FIXED mode, check if the touch is inside the base.
				if joystick_mode == Joystick_mode.FIXED and not _is_point_inside_base(event.position):
					return  # Ignore touches outside the base in FIXED mode.
				
				# For DYNAMIC/FOLLOWING, move the base to the touch position.
			...
			# Touch released and is our current joystick touch.
...
func _is_point_inside_joystick_area(point: Vector2) -> bool:
	# The joystick area is defined by this Control node's rectangle.
